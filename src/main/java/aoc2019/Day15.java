...
public class Day15 {
    ...
    public static void main(String[] args) {
        ...
        while (!q.isEmpty()) {
            ...
            visited.add(current);
            long[] memory = grid.get(current).getFirst();
            for (Direction dir : Direction.values()) {
                Point neighbour = dir.move(current);
                Pair<long[], Integer> res =
                        grid.computeIfAbsent(neighbour, k -> IntCode.run(memory, dir.value));
                int status = res.getSecond();
                if (status == 0) {
                    continue;
                }
                if (status == 2) {
        ...
        Map<Point, Integer> statuses = new HashMap<>();
        grid.forEach((k, v) -> statuses.put(k, v.getSecond()));
        ...
        while (!q.isEmpty()) {
            ...
            for (Direction dir : Direction.values()) {
                Point neighbour = dir.move(current);
                if (statuses.getOrDefault(neighbour, 0) == 0) {
                    continue;
                }
        ...
        int part1 = distances.get(station);
        System.out.println("Part 1: " + part1);
        int part2 = dfs(statuses, station, new HashSet<>());
        System.out.println("Part 2: " + part2);
    }

    public static Stream<Point> neighbours(Map<Point, Integer> grid, Point current) {
        return Arrays.stream(Direction.values())
                .map(dir -> dir.move(current))
                .filter(neighbour -> grid.getOrDefault(neighbour, 0) != 0);
    }

    private static int dfs(Map<Point, Integer> grid, Point current, Collection<Point> traversed) {
        traversed.add(current);
        return neighbours(grid, current).filter(point -> !traversed.contains(point))
                .mapToInt(neighbour -> dfs(grid, neighbour, traversed) + 1)
                .max()
                .orElse(0);
    }
